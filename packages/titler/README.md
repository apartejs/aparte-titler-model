...
const url = "https://huggingface.co/apartejs/aparte-titler/resolve/main/v1.2/titler-v1.2-latin-int3.bin";
...
const buf = await readFile("titler-v1.2-fr-int3.bin");
...
|---|---|---|
| `titler-v1.2-latin-int3.bin` | 17 European languages (en, fr, es, de, pt, it, nl, pl, sv, da, fi, cs, ro, no, hu, hr, lt) | 133 KB |
| `titler-v1.2-latin-mini-int3.bin` | the same 17, smaller vocabulary, −0.5 point on average | 96 KB |
| `titler-v1.2-efigsp-int3.bin` | en, fr, es, de, pt, it | 77 KB |
| `titler-v1.2-<lang>-int3.bin` | one language | 41 KB |

Every model also exists in `fp32`, `int8` and `int4`. `int3` is the recommended precision: 0.005 below `fp32` on the 17-language file, for the smallest file; `int4` scores as well as `fp32`. Scores, sizes and charts: [huggingface.co/apartejs/aparte-titler](https://huggingface.co/apartejs/aparte-titler).
